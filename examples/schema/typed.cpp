...
nucleus::registration_result define_space(Builder &builder)
{
    if(auto result = builder.register_element(nucleus::element("body", nucleus::anchor::root()));
       !result)
        return result;
    if(auto result = builder.register_element(nucleus::typed_element<vec3>(
               "pos", nucleus::anchor::keyspace("body"), make_vec3_converter()));
       !result)
        return result;
...
template<typename Builder>
nucleus::expected<nucleus::config_space, nucleus::error> make_space(Builder &builder)
{
...
int print_typed_values(const nucleus::config &config, std::ostream &output,
                       std::ostream &errors)
{
    auto pos = config.get_as<vec3>("body/pos");
    if(!pos)
    {
        errors << "get_as<vec3> failed: " << pos.error() << '\n';
        return 1;
    }
    output << "pos: " << pos.value().x << ", " << pos.value().y << ", " << pos.value().z << '\n';

    auto mass = config.get_as<int32_t>("body/mass");
    if(!mass)
    {
        errors << "get_as<int32_t> failed: " << mass.error() << '\n';
        return 1;
    }
    output << "mass: " << mass.value() << '\n';

    output << "pos (text): " << config.get("body/pos").value() << '\n';
...
int run_typed_example(nucleus::expected<nucleus::config_space, nucleus::error> space,
                      std::ostream &output, std::ostream &errors)
{
    ...
    auto loaded = nucleus::load_config(*space, nucleus::source_stack{},
                                       nucleus::load_options{.document_paths = {"config.xml"},
    ...
    return print_typed_values(loaded.value(), output, errors);
}
...
int main()
{
    return run_typed_example(make_space(), std::cout, std::cerr);
}
