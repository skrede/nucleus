#ifndef HPP_GUARD_NUCLEUS_CONFIGURATION_SOURCE_CONFIGURATION_SOURCE_REGISTRY_H
#define HPP_GUARD_NUCLEUS_CONFIGURATION_SOURCE_CONFIGURATION_SOURCE_REGISTRY_H
...
struct configuration_source_spec
{
    std::string name;
};
...
class configuration_source_registry
{
public:
    configuration_source_registry() = default;

    void add(configuration_source_spec spec, owner_token owner)
    {
    ...
    [[nodiscard]] const std::vector<registration<configuration_source_spec>> &entries() const noexcept
    {
        return m_entries;
    }

private:
    std::vector<registration<configuration_source_spec>> m_entries;
};
