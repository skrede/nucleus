...
// The convergence: a sealed config_space, a per-load source_stack,
// and the free load that folds a precedence stack with provenance and
// freezes an immutable config via a stack-local, const-borrowing context.
          ...
          "[resolution][lifecycle]")
{
    ...
    auto first = nucleus::load_config(space, nucleus::source_stack{one}, {});
    REQUIRE(first);
    auto second = nucleus::load_config(space, nucleus::source_stack{one}, {});
    REQUIRE(second);
